...
class AdminEntryController extends Controller
{
            ...
            ->when($search, function ($query) use ($search) {
                $query->where(function ($q) use ($search) {
                    $q->where('qty', 'like', "%{$search}%")
                        ...
                        ->orWhere('si_final', 'like', "%{$search}%")
                        ->orWhere('ba', 'like', "%{$search}%")
                        ->orWhere('ba_balik', 'like', "%{$search}%")
                        ->orWhere('no_inv', 'like', "%{$search}%")
                        ->orWhere('alamat_penerima_barang', 'like', "%{$search}%")
    ...
    public function store(Request $request, EntryPeriod $entry_period)
    {
        $validated = $request->validate([
            ...
            'harga_trucking' => 'nullable|string',
