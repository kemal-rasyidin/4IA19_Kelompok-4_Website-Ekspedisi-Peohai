<x-admin.layout>
    <div class="space-y-6">
        ...
        <div class="bg-white overflow-hidden shadow-md rounded-lg">
            <form id="bulkDeleteForm" action="{{ route('cities.bulkDestroy') }}" method="POST">
                @csrf
                @method('DELETE')
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <input type="checkbox" name="city_ids[]" value="{{ $city->id }}"
                                            class="city-checkbox w-4 h-4 text-blue-600 rounded focus:ring-blue-500">
                                    </td>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Event listener untuk setiap checkbox city
                cityCheckboxes.forEach(checkbox => {
                    checkbox.addEventListener('change', function() {
                        // Update status "Select All" checkbox
                        const allChecked = Array.from(cityCheckboxes).every(cb => cb.checked);
                        const someChecked = Array.from(cityCheckboxes).some(cb => cb.checked);
